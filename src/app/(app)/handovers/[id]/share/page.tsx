...
export default async function SharePage({
  ...
  if (handover.status === "DRAFT") {
    await prisma.handover.update({ where: { id }, data: { status: "ACTIVE" } });
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <Link href={`/handovers/${id}`} className="text-sm text-gray-500 hover:text-gray-900">
            ← Edit handover
          </Link>
        ...
        <div className="flex items-center gap-3">
          {handover.status !== "COMPLETE" && (
            <FinishHandoverButton handoverId={id} />
          )}
          {handover.status === "COMPLETE" ? (
      ...
      {handover.items.length === 0 ? (
        <p className="text-gray-500">No PRs in this handover.</p>
      ) : (
